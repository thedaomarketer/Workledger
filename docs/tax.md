...
- `lib/calculations/tax/` is a pure, framework-free calculation engine (no Supabase, no `server-only` imports) mirroring the rest of `lib/calculations/`:
  ...
  - Each `TaxLine` carries a stable `kind` (`federalIncomeTax`, `provincialIncomeTax`, `cpp`, ...) plus a `place` for regional lines; the UI renders `m.taxes.lines[kind]` so the breakdown is translated. The English `label` is kept for tests and logs.
